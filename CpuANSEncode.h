...
// 将频繁访问的变量提升到寄存器
    // const auto& entry = table[inBlock[k + idx1]];  // 确保table内存连续
    // const uint64_t pdf = entry.x;
    // const uint64_t magic = entry.z;
    // const uint32_t shift = entry.w;
    // const uint32_t offset = entry.y;
    
    // // 将state[k]加载到寄存器
    // uint64_t reg_state = state[k];  
    
    // // 分支消除：用位运算替代条件判断
    // const uint32_t write_mask = (reg_state >= (pdf << kStateCheckMul));
    // outWords[outOffset] = reg_state & kANSEncodedMask;
    // outOffset += write_mask;  // 注意：此处可能引入依赖，需评估
    
    // // 无分支右移
    // reg_state >>= (kANSEncodedBits & (-!!write_mask));  // write_mask为0时不移位
    
    // // 合并乘法和移位操作
    // const uint64_t tmp = reg_state * magic;
    // const uint64_t hi = tmp >> 32;
    // const uint64_t lo = tmp & 0xFFFFFFFF;
    // const uint64_t div = ((hi + reg_state) >> shift);
    
    // // 合并算术运算
    // reg_state = div * (one_bits - pdf) + offset + reg_state;
...
#undef RUN_ENCODE
  ...
  *headerOut = header;
  ...
  // uint32_t lastBlockWords = inSize % kDefaultBlockSize;
  // lastBlockWords = lastBlockWords == 0 ? kDefaultBlockSize : lastBlockWords;

...
public:
    ...
    __aicore__ inline void Init(TPipe *pipe, uint32_t datablockNum_H, uint32_t lastdataBlockNum, uint32_t elementNum_H, __gm__ uint8_t *in, __gm__ uint8_t *histogramDevice){
        this->pipe = pipe;
...
private:
    ...
    __aicore__ inline void Compute(LocalTensor<T> tempLocal, LocalTensor<T> maskLocal, LocalTensor<int32_t> &histogram, LocalTensor<int16_t> &histTensor0, LocalTensor<int32_t> &histTensor2, uint32_t computeNum) {
        ...
        for (int i = 0; i < HISTOGRAM_BINS; i++) {
            CompareScalar(maskLocal.template ReinterpretCast<uint8_t>(), inLocal.template ReinterpretCast<half>(), (histTensor0.template ReinterpretCast<half>())(i), CMPMODE::EQ, computeNum);
            GatherMask(tempLocal, tempLocal, maskLocal, true, computeNum, {1, 1, 1, 0}, rsvdCnt);
...
public:
    ...
    __aicore__ inline void Init(TPipe *pipe, __gm__ uint8_t* hist_in) {
        this->pipe = pipe;
        this->blockId = GetBlockIdx();
        this->blockNum = GetBlockNum();
...
__global__ __aicore__ void HistogramBF16(uint32_t datablockNum_H, uint32_t lastdataBlockNum, uint32_t elementNum_H, __gm__ uint8_t* in, __gm__ uint8_t* histogramDevice) {
    TPipe pipe;

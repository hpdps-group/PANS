...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint32_t i = blockId; i < datablockNum; i += 2 * blockNum)// 输入数据必须为32KB的整数倍
        {
            uint32_t offset = i * (computeNum * sizeof(uint16_t) / sizeof(T));
            CopyIn(offset);
            Compute0(i,
            ...
            compressedSize = compressedSize * sizeof(uint16_t);
            totalcompressedSize = totalcompressedSize + (uint32_t)compressedSize;
            CopyOut1(totalcompressedSize, compressedSize, i + blockNum);
        }
...
private:
                                ...
                                )
    {
        ...
        AIV_WITH_BARRIER(ShiftLeft, tempLocal0, tempLocal0, (uint32_t)24, tileNum);
        AIV_WITH_BARRIER(ShiftRight, tempLocal0, tempLocal0, (uint32_t)24, tileNum);
        ...
        AIV_WITH_BARRIER(ShiftRight, e_inLocal, e_inLocal, (uint32_t)8, computeNum);

...
public:
    ...
    __aicore__ inline void Init(TPipe* pipe,
        ...
        // assert(totaluncompressedbytes == 16777216);
...
public:
    __aicore__ inline void Process()
    {
        ...
        for(int i = 0; i < cyclenum; i ++){
            ...
            // TQueSync<PIPE_S, PIPE_MTE2> sync0;
        ...
        if (datatype == 0 | datatype == 1)
        {
            totalcompressedsize = 32 +
                                ...
                                datablocknum * (datablocksize / (tilelength * sizeof(uint16_t))) / 2 +
                                threadblocknum * 4 +
                                totalcompsize;
        }
        else
        {
            totalcompressedsize = 32 +
                                HISTOGRAM_BINS +
                                totaluncompressedbytes / 2 +
                                datablocknum * (datablocksize / (tilelength * sizeof(float))) / 2 +
                                threadblocknum * 4 +
                                totalcompsize;
        }
        ...
        if(blockId == 0){
            ...
            // header
            tempLocal[48](0) = datablocksize;                // dataBlockSize
            tempLocal[48](1) = datablocknum;                 // dataBlockNum
            tempLocal[48](2) = threadblocknum | (complevel << 16); // threadBlockNum (low 16), compLevel (high 16)
            tempLocal[48](3) = totaluncompressedbytes;       // totalUncompressedBytes
            // assert(tempLocal[48](3) == 16777216);
            tempLocal[48](4) = totalcompressedsize;          // totalCompressedBytes
            tempLocal[48](5) = tilelength | (datatype << 16); // tileLength (low 16), dataType (high 16)
            tempLocal[48](6) = mbllength | (options << 16);   // mblLength (low 16), options (high 16)
            tempLocal[48](7) = histogrambytes;               // histogramBytes

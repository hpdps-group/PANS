...
public:
    ...
    )
    {
        ...
        srcShape_prefix[0] = tileLength;
        srcShape_prefix[1] = tileNum;

        dstShape_offset[0] = tileLength;
        dstShape_offset[1] = 1;
        srcShape_offset[0] = tileLength;
        srcShape_offset[1] = tileNum;

        dstShape_mblcmp[0] = 1;
        dstShape_mblcmp[1] = tileNum / 8;
        srcShape_mblcmp[0] = tileLength;
        srcShape_mblcmp[1] = tileNum / 8;

        dstShape_div_fetch[0] = 1;
        dstShape_div_fetch[1] = tileNum;
        srcShape_div_fetch[0] = tileLength;
...
private:
                                ...
                                )
    {
        ...
        AIV_WITH_BARRIER(Muls, tempLocal2.template ReinterpretCast<int32_t>(), offset1Local.template ReinterpretCast<int32_t>(), (int32_t)outLocal(tileNum - 1), tileNum);
        ...
        AIV_WITH_BARRIER(ShiftRight, outLocal, outLocal, (uint32_t)14, computeNum);
...
//         srcShape_0[0] = 1;
//         srcShape_0[1] = tileNum;
//         dstShape_0[0] = tileLength;
//         dstShape_0[1] = tileNum;
...
//     {
//         LocalTensor<T> ms_inLocal = ms_inQueue.AllocTensor<T>();
//         AIV_WITH_BARRIER(DataCopy, ms_inLocal, ms_input[datablockId * (computeNum / 4)], computeNum / 4);
//         ms_inQueue.EnQue(ms_inLocal);
//     }
//     __aicore__ inline void CopyIn_mbl(uint32_t datablockId)
//     {
//         LocalTensor<T> mbl_inLocal = mbl_inQueue.AllocTensor<T>();
//         AIV_WITH_BARRIER(DataCopy, mbl_inLocal, mbl_input[datablockId * (tileNum / 8)], tileNum / 8);
//         mbl_inQueue.EnQue(mbl_inLocal);
//     }

//     __aicore__ inline void Compute(int32_t i,
//                                    uint32_t &accCompressed,
//                                    LocalTensor<T> &e_inLocal,
//                                    LocalTensor<T> &cmblLocal,
//                                    LocalTensor<T> &mergeLocal,
//                                    LocalTensor<T> &mblcmpLocal,
//                                    LocalTensor<T> &takeLocal,
//                                    LocalTensor<T> &tableLocal,
//                                    LocalTensor<T> &tempLocal0,
...
//         AIV_WITH_BARRIER(ShiftRight, mbl_inLocal[tileNum / 4], mbl_inLocal, (uint32_t)8, tileNum / 4);
//         AIV_WITH_BARRIER(ShiftRight, mbl_inLocal[tileNum / 2], mbl_inLocal, (uint32_t)4, tileNum / 2);
//         // AIV_WITH_BARRIER(ShiftLeft, tempLocal0, mbl_inLocal, (uint32_t)28, tileNum);
//         // AIV_WITH_BARRIER(ShiftRight, mbl_inLocal, tempLocal0, (uint32_t)28, tileNum);
//         AIV_WITH_BARRIER(And, mbl_inLocal, mbl_inLocal, mask15Local, tileNum * 2);

//         // AIV_WITH_BARRIER((Broadcast<float, 2, 0>), tempLocal0.template ReinterpretCast<float>(), cmblLocal.template ReinterpretCast<float>(), dstShape_0, srcShape_0);
...
//         AIV_WITH_BARRIER(ShiftLeft, mbl_inLocal, mbl_inLocal, (uint32_t)2, tileNum);
//         AIV_WITH_BARRIER(Gather, mbl_inLocal, takeLocal, mbl_inLocal, 0, tileNum);
//         // gather : 336GB/s compute: 260GB/s

//         // AIV_WITH_BARRIER((Broadcast<float, 2, 0>), tempLocal1.template ReinterpretCast<float>(), mbl_inLocal.template ReinterpretCast<float>(), dstShape_0, srcShape_0);
...
//         // AIV_WITH_BARRIER(Adds, outLocal.template ReinterpretCast<int32_t>(), outLocal.template ReinterpretCast<int32_t>(), (int32_t)28, computeNum);
//         // AIV_WITH_BARRIER(Select, outLocal.template ReinterpretCast<float>(), mblcmpLocal, outLocal.template ReinterpretCast<float>(),
//         //        (float)0, SELMODE::VSEL_TENSOR_SCALAR_MODE, computeNum);
...
//     {
//         LocalTensor<T> outLocal = outQueue.DeQue<T>();
//         AIV_WITH_BARRIER(DataCopy, output[datablockId * (datablockSize / sizeof(T))], outLocal, datablockSize / sizeof(T));
//         outQueue.FreeTensor(outLocal);
//     }

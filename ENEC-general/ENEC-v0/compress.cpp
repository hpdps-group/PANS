...
extern "C" void enec_compress(Header *cphd, void *stream, uint8_t *srcDevice, uint8_t *compressedDevice, uint8_t *histogramDevice, uint8_t *blockCompSizeDevice);
...
int main(int32_t argc, char *argv[])
{
    ...
    {
        auto start = std::chrono::high_resolution_clock::now();
        enec_compress(cphd, stream, srcDevice, compressedDevice, histogramDevice, blockCompSizeDevice);
        CHECK_ACL(aclrtSynchronizeStream(stream));
        auto end = std::chrono::high_resolution_clock::now();
        if (i > 5)
            comp_time +=

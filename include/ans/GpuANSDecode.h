...
void ansDecode(
  ...
  uint32_t* table_dev;
  CUDA_VERIFY(hipMalloc(&table_dev, (1 << precision) * sizeof(uint32_t)));
  ...
  // uint32_t table_host[1<<precision];
  // hipMemcpy(table_host, table_dev, (1<<precision) * sizeof(uint32_t), hipMemcpyDeviceToHost);

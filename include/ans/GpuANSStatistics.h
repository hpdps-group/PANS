 ...
 */
#ifndef MULTIBYTE_ANS_INCLUDE_ANS_GPUANSSTATISTICS_H
#define MULTIBYTE_ANS_INCLUDE_ANS_GPUANSSTATISTICS_H
...
//typename InProvider, 
int Threads>
__global__ void histogramBatch(
  //InProvider in, 
  uint8_t* in_dev,
  uint32_t inSize_dev,
  uint32_t* out) {
...
#pragma unroll
  for (int i = 0; i < kNumSymPerThread; ++i) {
    ...
    //这个指令对于各种算法优化非常有用，
    //比如快速确定优先级掩码中最高优先级或者计算操作数规范化时需要左移的位数
    //假设有一个32位整数0x1FFFFFFF，
    //其二进制表示为0001 1111 1111 1111 1111 1111 1111 1111。
    //使用__clz指令后，
...
void ansHistogramBatch(
  ...
  CUDA_VERIFY(cudaMemsetAsync(
      histogram_dev, 0, sizeof(uint32_t) * kNumSymbols * 1, stream));

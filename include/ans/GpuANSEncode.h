...
__global__ void ansEncodeBatch(
  ...
  int grim_warp_numid =//这个grim_warp_numid指的是一个grim中的全局warpid
    //   __shfl_sync(0xffffffff, 
      (blockIdx.x * blockDim.x + tid) / kWarpSize
    //   , 0)
      ;
...
//             true, state, symV[j] & 0xff, outOffset, outWords, smemLookup);
...
__global__ void ansEncodeCoalesceBatch(
  ...
  if (block == 0) {
    if (tid == 0) {
      ...
      // printf("compressedWordsPrefix_dev[numBlocks - 1]: %d, compressedWords_dev[numBlocks - 1]: %d, totalCompressedWords: %d\n", compressedWordsPrefix_dev[numBlocks - 1], compressedWords_dev[numBlocks - 1], totalCompressedWords);
...
#undef RUN_ENCODE
  ...
  // uint32_t compressedWords_host[maxNumCompressedBlocks];
  // hipMemcpy(compressedWords_host, compressedWordsPrefix_dev, sizeof(uint32_t) * maxNumCompressedBlocks, hipMemcpyDeviceToHost);
  // for(int i = 0; i < 10; i ++){
  //   printf("i: %d, compressedWords_dev: %d\n", i, compressedWords_host[i]);
  // }
  //   printf("last: %d\n", inSize % kDefaultBlockSize);
  {

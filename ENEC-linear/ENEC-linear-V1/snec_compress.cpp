...
//         // if(i == 20){
//         //     DumpTensor(mbl_outLocal, 1, tileNum);
//         // }
...
//         AIV_WITH_BARRIER(CompareScalar, tempLocal0.template ReinterpretCast<uint8_t>(), tempLocal0.template ReinterpretCast<float>(),
...
//         AIV_WITH_BARRIER(Select, mergeLocal.template ReinterpretCast<float>(), tempLocal0, e_inLocal.template ReinterpretCast<float>(),
...
public:
                                ...
                                __gm__ uint8_t *blockCompSize)
    {
        ...
        srcShape_cmp[0] = 1;
        srcShape_cmp[1] = tileNum / 8 / sizeof(T);
        dstShape_cmp[0] = tileLength;
        dstShape_cmp[1] = tileNum / 8 / sizeof(T);

        srcShape_pow[0] = 1;
        srcShape_pow[1] = tileNum;
        dstShape_pow[0] = tileLength;
        dstShape_pow[1] = tileNum;
    ...
    __aicore__ inline void Process()
    {
        ...
        uint64_t compressedSize = 0;
        uint32_t totalcompressedSize = 0;
        uint32_t cycle_stride = (blockNum << 1);
        for (uint32_t i = blockId; i < datablockNum; i += cycle_stride)
        {
            uint32_t offset = i * (computeNum * sizeof(uint16_t) / sizeof(T));
            CopyIn(offset);
            Compute0(i,
                    ...
                    mask15Local,
                    compareMask);
            CopyOut0(totalcompressedSize, compressedSize, i);
            offset = (i + blockNum) * (computeNum * sizeof(uint16_t) / sizeof(T));
            CopyIn(offset);
            Compute1(i,
                    compressedSize,
                    tempLocal0,
            ...
            compressedSize = compressedSize * sizeof(uint16_t);
            totalcompressedSize = totalcompressedSize + (uint32_t)compressedSize;
            CopyOut1(totalcompressedSize, compressedSize, i);
        }
...
private:
    ...
    __aicore__ inline void Compute0(uint32_t i,
                                   uint64_t &compressedSize,
                                   LocalTensor<T> &tempLocal0,
                                   LocalTensor<T> &tableLocal,
                                   LocalTensor<T> &mergeLocal,
                                ...
                                )
    {
        ...
        AIV_WITH_BARRIER(ShiftRight, e_outLocal.template ReinterpretCast<uint16_t>(), e_inLocal.template ReinterpretCast<uint16_t>(), (uint16_t)15, computeNum);
        AIV_WITH_BARRIER(ShiftLeft, e_inLocal.template ReinterpretCast<uint16_t>(), e_inLocal.template ReinterpretCast<uint16_t>(), (uint16_t)1, computeNum);
        AIV_WITH_BARRIER(Or, e_inLocal.template ReinterpretCast<uint16_t>(), e_inLocal.template ReinterpretCast<uint16_t>(), e_outLocal.template ReinterpretCast<uint16_t>(), computeNum);

        AIV_WITH_BARRIER(ShiftLeft, e_outLocal.template ReinterpretCast<uint16_t>(), e_inLocal.template ReinterpretCast<uint16_t>(), (uint16_t)8, computeNum);
        AIV_WITH_BARRIER(ShiftRight, e_outLocal.template ReinterpretCast<uint16_t>(), e_outLocal.template ReinterpretCast<uint16_t>(), (uint16_t)8, (int)(computeNum / 2));
        AIV_WITH_BARRIER(Or, ms_outLocal.template ReinterpretCast<uint16_t>(), e_outLocal.template ReinterpretCast<uint16_t>(), (e_outLocal.template ReinterpretCast<uint16_t>())[computeNum / 2], computeNum / 2);
        ...
        AIV_WITH_BARRIER(Adds, e_inLocal.template ReinterpretCast<int32_t>(), e_inLocal.template ReinterpretCast<int32_t>(), (int32_t)(-127), computeNum);
        ...
        if(tileLength >= 4)
            AIV_WITH_BARRIER(Or, tempLocal0, tempLocal0, tempLocal0[computeNum / 4], computeNum / 4 * 2);
        if(tileLength >= 8)
            AIV_WITH_BARRIER(Or, tempLocal0, tempLocal0, tempLocal0[computeNum / 8], computeNum / 8 * 2);
        if(tileLength >= 16)
            AIV_WITH_BARRIER(Or, tempLocal0, tempLocal0, tempLocal0[computeNum / 16], computeNum / 16 * 2);
        if(tileLength >= 32)
            AIV_WITH_BARRIER(Or, tempLocal0, tempLocal0, tempLocal0[computeNum / 32], computeNum / 32 * 2);
        ...
        Adds(tempLocal0.template ReinterpretCast<int32_t>(), tempLocal0.template ReinterpretCast<int32_t>(), (int32_t)(127), tileNum);
        ShiftLeft(tempLocal0, tempLocal0, (uint32_t)23, tileNum);
        Cast(tempLocal0.template ReinterpretCast<int32_t>(), tempLocal0.template ReinterpretCast<float>(), RoundMode::CAST_TRUNC, tileNum);
        AIV_WITH_BARRIER((Broadcast<float, 2, 0>), tempLocal0.template ReinterpretCast<float>(), tempLocal0.template ReinterpretCast<float>(), dstShape_pow, srcShape_pow);// 比特位长进行广播
    ...
    __aicore__ inline void Compute1(uint32_t i,
                                   uint64_t &compressedSize,
                                   LocalTensor<T> &tempLocal0,
                                   LocalTensor<T> &tableLocal,
                                   LocalTensor<T> &mergeLocal,
                                ...
                                )
    {
        ...
        AIV_WITH_BARRIER(CompareScalar, compareMask.template ReinterpretCast<uint8_t>(), cmblLocal.template ReinterpretCast<float>(),
                      (mask15Local.template ReinterpretCast<float>())(0), CMPMODE::GT, tileNum);
        // if(i == 0){
        //     DumpTensor(cmblLocal, 1, tileNum);
        // }
        AIV_WITH_BARRIER(And, cmblLocal, cmblLocal, mask15Local, tileNum * 2);

        AIV_WITH_BARRIER((Broadcast<float, 2, 0>), compareMask.template ReinterpretCast<float>(), compareMask.template ReinterpretCast<float>(), dstShape_cmp, srcShape_cmp);// compareMask进行广播

        // AIV_WITH_BARRIER(Cast, tempLocal0.template ReinterpretCast<float>(), tempLocal0.template ReinterpretCast<int32_t>(), RoundMode::CAST_TRUNC, tileNum);
        // AIV_WITH_BARRIER(Power, e_outLocal.template ReinterpretCast<float>(), (float)2, tempLocal0.template ReinterpretCast<float>(), e_outLocal.template ReinterpretCast<uint8_t>(), tileNum);
        // AIV_WITH_BARRIER(Cast, tempLocal0.template ReinterpretCast<int32_t>(), e_outLocal.template ReinterpretCast<float>(), RoundMode::CAST_TRUNC, tileNum);
    ...
    __aicore__ inline void CopyOut0(uint32_t totalcompressedSize, uint32_t compressedSize, uint32_t datablockId)
    {
        LocalTensor<T> e_outLocal = e_outQueue.DeQue<T>();
        LocalTensor<T> mbl_outLocal = mbl_outQueue.DeQue<T>();
        LocalTensor<T> ms_outLocal = ms_outQueue.DeQue<T>();
    ...
    __aicore__ inline void CopyOut1(uint32_t totalcompressedSize, uint32_t compressedSize, uint32_t datablockId)
    {
        LocalTensor<T> e_outLocal = e_outQueue.DeQue<T>();
        LocalTensor<T> mbl_outLocal = mbl_outQueue.DeQue<T>();
        LocalTensor<T> ms_outLocal = ms_outQueue.DeQue<T>();
...
private:
    ...
    uint32_t srcShape_pow[2];
    uint32_t dstShape_pow[2];
};
